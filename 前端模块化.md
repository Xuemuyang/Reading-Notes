...
`Node.js`诞生标志JS模块化编程诞生。`Node.js`首先采用了js模块化的概念。
...
根据AMD规范，我们使用`define`定义模块，使用`require`调用模块。

是在浏览器端模块化开发的规范。

AMD是`require.js`在推广过程中对模块定义的规范化的产出。
...
主模块的意思是网页的入口，有点像C语言的`main()`函数，所有代码从这里开始运行。

主模块会依赖于其他模块，这时就要使用AMD规范定义的`require()`函数。
