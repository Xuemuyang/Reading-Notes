...
若`HTTP`首部字段重复了，规范可能还未明确，浏览器处理的情况不一致。
...
`HTTP`提供了几个对已缓存对象再验证的工具，最常见的是`If-Modified-Since`首部。只有在本地缓存了对象，而服务器又对缓存对象作出修改的情况下，服务器才发送此对象。
...
HTTP没有为用户提供一种手段来区分响应式缓存命中的还是访问原始服务器得到的。两种情况响应码都是200，客户端可以使用Date首部来判断响应是否来自缓存。如果响应中的日期值比较早，客户端就可以认为这是一条缓存的响应。

### 缓存的处理步骤

1. 接受---缓存从网络中读取抵达的请求报文
1. 解析---缓存对报文进行解析，提取出URL和各种首部
1. 查询---缓存查看是否有本地副本可用，如果没有，就获取一份副本(并将其保存在本地)
1. 新鲜度检测---缓存查看已缓存副本是否足够新鲜，如果不是，就询问服务器是否有任何更新
1. 创建响应---缓存会用新的首部和已缓存的主体来构建一条响应报文。
1. 发送---缓存通过网络将响应发回客户端
1. 日志---缓存可选地创建一个日志文件条目来描述这个事务
...
`Cache-Control`首部使用的是相对时间而不是绝对时间，更倾向于使用比较新的`Cache-Control`字段。
...
#### 什么时候使用Etag和Last-Modified

如果HTTP/1.1缓存或服务器收到的请求既带有`If-Modified-Since`又带有实体标签条件首部，只有这两个条件都满足才返回304 Not Modified。

### 控制缓存的能力

服务器可以指定文档的缓存时间，按照优先级递减的顺序:
...
标识为`no-cache`的响应可以存储在本地缓存区中，在于原始服务器进行新鲜度再验证前，缓存不能将其提供给客户端使用。

`max-age`首部标识从服务器将文档传来之时，可以认为此文档出于新鲜状态的秒数。

服务起可以请求缓存不要缓存文档，或者将最大使用期限设置为零，从而在每次访问的时候都进行刷新。

Cache-Control: max-age=0

#### 客户端对新鲜度的控制

Web浏览器有Refesh(刷新)或Reload(重载)，Refresh按钮会发布一个附加了`Cache-Control`请求首部的GET请求。

客户端通过`Cache-Control`请求首部来强化或放松对过期时间的限制。

##### 浏览器刷新行为

+ 普通刷新会忽略`Cache-Control`，但不会清楚它，需要强制刷新
+ 在地址栏重新输入当前页面地址并按下回车也会当做刷新处理
+ 浏览器强制刷新请求会带上`Cache-Control:no-cache`和`Pragma:no-cache`
+ 普通刷新会启用协商缓存，忽略强缓存
+ 从地址栏或收藏夹输入网址，通过连接引用资源等情况下，浏览器才会启用强缓存

#### 设置缓存控制

##### 通过HTTP-EQUIV控制HTML缓存

结论就是，`<META HTTP-EQUIV>`标签并不是控制文档缓存特性的好方法，通过配置正确的服务器发出HTTP首部，是传送文档缓存控制请求的唯一可靠办法。
...
一般`Form`首部是由自动化机器人或者蜘蛛发送。

根据`User-Agent`实现浏览器兼容。

`Referer`首部自身不能完全标识用户，通过它可以更好理解用户的浏览行为，以及用户兴趣。
